...
** Last update Sat Jan 19 09:22:16 2013 louis-philippe baron
*/
...
void		move_forward(t_all *all)
{
  ...
  x = all->player->x_player + (all->maths->cos[angle] * (1.0 / 10.0)) + 0.1;
  y = all->player->y_player + (all->maths->sin[angle] * (1.0 / 10.0)) + 0.1;
...
void		strafe_q(t_all *all)
{
  ...
  if (angle >= 360)
    angle -= 360;
  x = all->player->x_player + (all->maths->cos[angle] * (80.0 / 1000.0)) + 0.1;
  y = all->player->y_player + (all->maths->sin[angle] * (80.0 / 1000.0)) + 0.1;
  if ((x >= 0 && x < all->map->x_max && y >= 0 && y < all->map->y_max
       && all->map->map[(int)y][(int)x] != '1') ||
      (x < 0 || y < 0 || x >= all->map->x_max || y >= all->map->y_max))
    {
